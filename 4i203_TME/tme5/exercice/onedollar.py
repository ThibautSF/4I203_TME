...
class OneDollar(object):
    ...
    def recognize(self, points):
        ...
        score = 1-b/(0.5*numpy.sqrt(self.square_size**2+self.square_size**2))
    ...
    def addTemplate(self, template, label):
        ...
        self.sampled_templates.append(points)
        points = self.resample(points, numPoints)
    ...
    def rotateToZero(self, points):
        ...
        #todo
        angle = numpy.arctan2(centroid[0]-points[0][0],centroid[1]-points[0][1])
        newPoints = self.rotateBy(points, -angle)
    ...
    def rotateBy(self, points, angle):
        ...
        for p in points:
            q = np.array([0., 0.])
            q[0] = (p[0]-centroid[0])*numpy.cos(angle)-(p[1]-centroid[1])*numpy.sin(angle)+centroid[0]
            q[1] = (p[0]-centroid[0])*numpy.sin(angle)+(p[1]-centroid[1])*numpy.cos(angle)+centroid[1]
            newPoints = numpy.append(newPoints, [q], 0)
    ...
    def scaleToSquare(self, points):
        ...
        #todo 7
        maxp = numpy.max(points, 0)
        minp = numpy.min(points, 0)
        ...
        for p in points:
            q = np.array([0., 0.])
            q[0] = p[0]*(self.square_size/width)
            q[1] = p[1]*(self.square_size/height)
            newPoints = numpy.append(newPoints, [q], 0)
